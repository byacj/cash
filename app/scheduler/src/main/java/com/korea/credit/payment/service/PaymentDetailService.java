...
public class PaymentDetailService {
    ...
    public void transfer(List<TransferType> transferTypes) {
        ...
        List<TransferMessageHistory> transferMessageHistories =
                transferMessageHistoryRepository.findByTypeInAndStatus(transferTypes, TransferStatus.REQUEST);

        log.info("process transfer. count: " + transferMessageHistories.size());

        transferMessageHistories.forEach(transferMessageHistory -> {
            LocalDate endDate = transferMessageHistory.getProceedEndDate();
            ...
            jmsTemplate.convertAndSend(ActiveMQConfiguration.PAYMENTS, paymentDetails);
